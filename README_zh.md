...
## 📄介绍
RagPanel是一个开源RAG快速部署项目，仅用几行代码就可以快速部署数据库和RAG平台，可通过可视化Web UI进行添加文件、检索查询、删除检索结果、删除已添加文件、RAG聊天等操作，并支持通过API调用完成这些操作，目前已集成了Naive RAG、Graph RAG等RAG方式。项目特色如下：
### 用户友好
RagPanel部署简便，仅需几行命令就能快速完成，同时提供了可视化Web UI，在部署完成后不需要额外代码就能完成本地知识库的构建和问答。
### 调试便捷
RagPanel在Web UI提供了添加文档、检索数据库、修改数据库等调试功能，以便用户在问答前检查文档的分割、检索和重排序情况。
### 高自由度
RagPanel在RAG流程的各个部分都提供了较多可更改的配置，用户在默认配置的基础上还能根据自身需求来自由修改。
## 🚀快速开始
### 准备模型
